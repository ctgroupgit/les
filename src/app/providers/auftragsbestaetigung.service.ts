  ...
  generate(data = [], type: string) {
    ...
    data.forEach( (row) => {
      switch (row[1]) {
        ...
        case 'KOPF_ANS': {
          docClass.docHeadingDetail.ourContact.push('-B' + row[20]);
          docClass.docHeadingDetail.ourContact.push(row[21]);
          docClass.docHeadingDetail.ourContact.push(row[22]);
          docClass.docHeadingDetail.ourContact.push(row[23]);
          docClass.docHeadingDetail.ourContact.push(row[24]);
          break;
        }
        case 'KOPF_RGAN': {
          docClass.docHeadingDetail.deliveryContact.push('-B' + row[20]);
          docClass.docHeadingDetail.deliveryContact.push(row[21]);
          docClass.docHeadingDetail.deliveryContact.push(row[22]);
          docClass.docHeadingDetail.deliveryContact.push(row[23]);
          docClass.docHeadingDetail.deliveryContact.push(row[24]);
          break;
        }
        case 'KOPF': {
          ...
          docClass.dh1.title = row[15].trim();
          docClass.dh1.description = row[16].trim();
        ...
        case 'POS': {
          ...
          this.bodyRow.col2.priceDetailDescription = 'Termin/Date';
          this.bodyRow.col2.priceDetailValue = row[12];
          if (parseFloat(row[7]) > 0) {
            this.bodyRow.lastLine = this.gb.normalizeChar('netto:' + row[7], 25);
          }
